...
public class Cone extends ShapeKey {
    ...
    public void run(Location location, ParticleMaker maker) {
        ...
        for (int a=0; a<render_times; a++) {
            for (int i = 0; i < points; i++) {
                ...
                maker.sendToLocation(location.add(vector).add(0.0D, y, 0.0D));

                location.subtract(vector).subtract(0.0D, y, 0.0D);
                if (y < 3.0F) {
